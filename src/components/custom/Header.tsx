...
const Header = () => {
    ...
    return (
        <header className="bg-primary relative flex justify-between items-center lg:h-20">
            <Logo link={true} size={'sm'} />
            <BtnMenu className={'z-20 lg:hidden'} onClick={toggleMenu} isOpen={!isOpen} size={32} />
            <section className={`${isOpen ? 'h-full' : 'hidden h-auto lg:flex lg:items-center'} bg-primary w-full fixed top-0 left-0 z-10 lg:z-0 lg:static transform transition-transform duration-1000 ease-in-out`}>
                <Nav className={'h-1/2'} />
